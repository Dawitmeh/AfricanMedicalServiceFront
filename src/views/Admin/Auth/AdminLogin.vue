           ...
           <div>
              ...
              <form @submit="login" class="space-y-4 md:space-y-6" action="#">
                  <div>
                      ...
                      <input type="email" v-model="admin.email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-theme-main focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required="">
                  ...
                  <div>
                      ...
                      <input type="password" v-model="admin.password" name="password" id="password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="">
                        <div v-if="errMsg2?.email" class="text-red-500 text-xs mt-1">
                            {{ errMsg2?.email[0] }}
                        </div>
                  </div>
                  ...
                  <button type="submit" class="w-full text-white bg-theme-main hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Sign in</button>
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      Create an account <router-link :to="{name: 'Register'}" class="font-medium text-theme-main hover:underline dark:text-primary-500">Sign up Here</router-link>
                  </p>
              </form>
          </div>
      </div>
...
function login(e) {
    ...
    if (validateForm()) {
        ...
        store.login({ ...admin.value })
            .then(() => {
                if (store.admin.user_type === 'admin') {
                    router.push(
                        {name: 'AdminDashboard'}
                    )
                } else {
                    store.notify({
                        type: 'error',
                        message: 'You are not allowed to access admin dashboard'
                    })
                    router.push({
                        name: 'AdminLogin'
                    })
