            ...
            <div class="mx-auto max-w-screen-lg px-4 2xl:px-0">
                ...
                <div class="mb-4 grid gap-4 sm:grid-cols-2 sm:gap-8 lg:gap-16">
                    ...
                    <div class="flex space-x-4">
                        <img class="h-16 w-16 rounded-lg" :src="client?.image" alt="Helene avatar" />
                        <div>
                        <span class="mb-2 inline-block rounded bg-primary-100 px-2.5 py-0.5 text-xs font-medium text-primary-800 dark:bg-primary-900 dark:text-primary-300"> {{ client?.status }} </span>
                        <h2 class="flex items-center text-xl font-bold leading-none text-gray-900 dark:text-white sm:text-2xl">{{ client?.first_name }} {{ client?.last_name }}</h2>
                        </div>
                    </div>
                    <dl class="">
                        <dt class="font-semibold text-gray-900 dark:text-white">Email Address</dt>
                        <dd class="text-gray-500 dark:text-gray-400">{{ client?.email }}</dd>
                    </dl>
                    <dl>
                    ...
                    <div class="space-y-4">
                    <dl>
                        <dt class="font-semibold text-gray-900 dark:text-white">Country Code</dt>
                        <dd class="flex items-center gap-1 text-gray-500 dark:text-gray-400">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="hidden h-5 w-5 shrink-0 text-gray-400 dark:text-gray-500 lg:inline">
                    ...
                    <dl>
                        ...
                        <dd class="text-gray-500 dark:text-gray-400">{{ client?.phone }}</dd>
                    </dl>
                    <dl>
                        <dt class="font-semibold text-gray-900 dark:text-white">User Type</dt>
                        <dd class="text-gray-500 dark:text-gray-400">{{ client?.user?.userType }}</dd>
                    </dl>
                    </div>
                </div>
                <button @click="onRemove(client?.id)" type="button" data-modal-target="accountInformationModal2" data-modal-toggle="accountInformationModal2" class="inline-flex w-full items-center justify-center rounded-lg bg-red-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800 sm:w-auto">
                     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="-ms-0.5 me-1.5 h-4 w-4">
...
const client = computed(() => store.currentClient)
...
onBeforeMount(() => {
    if (route.params.id) {
        store.getClient(route.params.id)
    }
})
</script>