    ...
    <AdminPageComponent>
        ...
        <div v-else class="relative overflow-x-auto shadow-md sm:rounded-lg mt-9">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr v-for="(product, index) in products" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="w-4 p-4">
                            <div class="flex items-center">
                        ...
                        <td class="px-6 py-4">
                            {{ product?.cost }} {{  product?.currency?.name }}
                        </td>
                        <td class="px-6 py-4">
                            {{ product?.price }} {{  product?.currency?.name }}
                        </td>
                        <td class="px-6 py-4">
                            <div v-if="product.active == 1" class="flex items-center">
                        ...
                        <td class="px-6 py-4">
                            <!-- Modal toggle -->
                            <button @click="openEditModal(product.id)" type="button" data-modal-target="editUserModal" data-modal-show="editUserModal" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                      ...
                      <div v-if="errMsg != false" id="alert-border-2" class="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 dark:text-red-400 dark:bg-gray-800 dark:border-red-800" role="alert">
                        ...
                        <div class="grid gap-4 mb-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" v-model="product.name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
                                <div v-if="validationErr.name" class="text-red-500 text-xs mt-1">
                                  {{ validationErr.name[0] }}
                                </div>
                            </div>
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Estimated Cost</label>
                                <input type="text" v-model="product.cost" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
                                <div v-if="validationErr.cost" class="text-red-500 text-xs mt-1">
                                  {{ validationErr.cost[0] }}
                                </div>
                            </div>
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Estimated Price</label>
                                <input type="text" v-model="product.price" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
                                <div v-if="validationErr.price" class="text-red-500 text-xs mt-1">
                                  {{ validationErr.price[0] }}
                                </div>
                            </div>
                            <div>
                                <label for="category" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Currency</label>
                                <select id="category" v-model="product.currency_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    <option selected="">Select currency</option>
                                    <option v-for="currency in currencies" :key="currency.id" :value="currency.id">
                                        {{ currency?.name }}
                                    </option>
                                </select>
                                <div v-if="validationErr.currency_id" class="text-red-500 text-xs mt-1">
                                    {{ validationErr?.currency_id[0] }}
                                </div>
                            </div> 
                            <div>
                                <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
                                <textarea id="description" v-model="product.description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write product description here"></textarea>                    
...
const currencies = computed(() => countryStore.currencies)
...
function validateForm() {
    ...
    if (!product.value.currency_id) {
        validationErr.value.currency_id = ['Currency is required']
    }
    if (!product.value.name) {
        validationErr.value.name = ['Name is required']
    }
    if (!product.value.cost) {
        validationErr.value.cost = ['Cost is required']
    }
    if (!product.value.price) {
        validationErr.value.price = ['Price is required']
    }
...
function saveProduct(e) {
    ...
    if (validateForm()) {
        ...
        const apiCall = isEditing.value ? store.updateProduct : store.createProduct
        apiCall({...product.value})
            .then(() => {
                store.notify({
                    type: 'success',
                    message: 'The product was successfully ' + action
                })
                closeModal()
                store.getProducts()
            })
